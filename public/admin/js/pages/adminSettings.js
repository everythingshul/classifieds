async function renderSettingsPage() {
  ...
  root.innerHTML = `
    ...
    <div class="admin-card">
      <h3 style="margin-top:0">Outbound Email — SMTP</h3>
      <p class="hint">Leave Host blank to log emails to the server console instead of sending them (useful for testing).</p>
      <form id="smtpForm">
        <div class="form-cols">
    ...
    <div class="admin-card">
      ...
      <form id="rulesForm">
        ...
        <div class="form-cols">
          <div class="form-row"><label>Title character limit</label><input name="char_limit_title" type="number" value="${s.char_limits?.title ?? 80}"></div>
          <div class="form-row"><label>Description character limit</label><input name="char_limit_description" type="number" value="${s.char_limits?.description ?? 1200}"></div>
        </div>
        <button class="btn btn-sm" type="submit">Save</button>
      </form>
    </div>
  ...
  bindForm('smtpForm', (fd) => {
    const out = [
      ['smtp_host', fd.smtp_host], ['smtp_port', fd.smtp_port], ['smtp_user', fd.smtp_user],
      ['mail_from', fd.mail_from], ['smtp_secure', fd.smtp_secure === '1'],
    ];
  ...
  bindForm('rulesForm', (fd) => [
    ...
    ['char_limits', { title: Number(fd.char_limit_title), description: Number(fd.char_limit_description) }],
  ]);
  bindForm('locForm', (fd) => [
